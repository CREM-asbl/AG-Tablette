...
class TangramPopup extends LitElement {
    ...
    selectTangram(type, id) {
        ...
        let actions = app.workspace.shapes.map(shape => {
            let delAction = new DeleteAction('delete_shape');
            delAction.shape = shape;
            return delAction;
        }).concat(tangram.shapes.map(shape => {
    ...
    updateTangramsList() {
        ...
        for(let i=0; i<2; i++) {
            ...
            ul.innerHTML = html;
            [...ul.children].filter(li => li.nodeName.toLowerCase()=='li').forEach((li, i) => {
                li.onclick = () => {
    ...
    _actionHandle(event) {
        ...
        switch (event.target.name) {
            ...
            default:
                console.log("Tangram popup: paramètre inconnu: " + event.target.name + ' ' + event.target.value + ' ' + event.target.checked);
        }
    }
