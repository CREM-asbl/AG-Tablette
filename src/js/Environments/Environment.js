...
export class Environment {
    ...
    checkKitStructure(kit) {
        for(let familyName of Object.keys(kit)) {
            ...
            if(!familyData.color || !familyData.shapes) { //(1)
                console.error(familyName + " - bad kit structure");
                return false;
            }
            if(!familyData.shapes.every(shape => {
                if(!shape.name || !shape.steps) { //(2)
                    console.error(familyName + " - bad kit structure");
                    return false;
                }
                if(!shape.steps.every(step => {
                    if(!step.type || !Number.isFinite(step.x)
                            || !Number.isFinite(step.y)) { //(3)
                        console.error(familyName + " - bad kit structure");
                        return false;
                    }
                    if(step.type != 'moveTo' && step.type != 'vertex'
                            && step.type != 'segment') { //(3)
                        console.error(familyName + " - bad kit structure");
                        return false;
                    }
                    if(step.type=='segment' && step.isArc !== undefined &&
                            step.isArc !== false && step.isArc !== true) { //(3)
                        console.error(familyName + " - bad kit structure");
                        return false;
                    }
                    if(step.type!='segment' && step.isArc !== undefined) { //(3)
                        console.error(familyName + " - bad kit structure");
                        return false;
                    }
                    return true;
                    ...
                    return false;
                if(shape.steps[0].type!='moveTo') { //(4)
                    console.error(familyName + " - bad kit structure");
                    return false;
                }
                for(let i=1; i<shape.steps.length; i++) {
                    ...
                    if(s1.type=='vertex' && s2.type=='vertex') {
                        if(s1.isArc !== true && s2.isArc !== true) { //(5)
                            console.error(familyName + " - bad kit structure");
                            return false;
                        }
                    }
                }
                if(shape.steps.length<2) { //(6)
                    console.error(familyName + " - bad kit structure");
                    return false;
                }
                return true;
