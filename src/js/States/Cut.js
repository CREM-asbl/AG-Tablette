...
export class CutState extends State {
    ...
    objectSelected(object, clickCoordinates, event) {
        if(this.currentStep == "listen-canvas-click") {
            ...
            if(object.pointType=='center') {
                ...
                if(!this.isLineValid(object.shape, pt1Coord, pt2Coord))
                    return;
    ...
    isLineValid(shape, pt1, pt2) {
        ...
        for(let i=1; i<amountOfParts; i++) {
            ...
            if(!app.drawAPI.isPointInShape(pt, shape))
                return false;
            atLeastOneNotInBorder =
                atLeastOneNotInBorder || !shape.isPointInBorder(pt);
        }
    ...
    setSelConstraints(step) {
        ...
        if(step == 'listen-canvas-click') {
            ...
            }).concat(segmentsToAdd.map(segment => {
                return bs[segment].points.filter(pt => {
