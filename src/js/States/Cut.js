...
import {settings} from '../Settings'
 ...
 export class CutState {
    ...
    click(point, selection) {
        ...
        var list = app.workspace.pointsNearPoint(new Point(point.x, point.y, null, null))
        ...
        let pointObj = pointsPossibles[0]
     ...
     */
    getElementsToHighlight(overflownShape, mouseCoordinates) {
        var data = {
            'shapes': [],
            'segments': [],
            'points': []
        ...
        if (!this.shape) {
            ...
            var list = app.workspace.pointsNearPoint(new Point(mouseCoordinates.x, mouseCoordinates.y, null, null));
            list.reverse();
...
addEventListener('app-loaded', () => window.app.states.cut_shape = new CutState())