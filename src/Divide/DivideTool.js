...
export class DivideTool extends Tool {
  ...
  setSelectionConstraints() {
    ...
    if (app.tool.currentStep == 'selectObject') {
      ...
      app.workspace.selectionConstraints.segments.blacklist = app.mainCanvasElem.shapes
  ...
  objectSelected(object) {
    if (app.tool.currentStep == 'selectObject') {
      ...
      let pt1 = app.mainCanvasElem.findObjectById(
      ...
      if (pt1.coordinates.dist(object1.coordinates) < 0.01) {
        ...
        app.upperCanvasElem.removeObjectById(
          app.upperCanvasElem.points[0].id,
          'point',
        );
      ...
      //   window.dispatchEvent(new CustomEvent('show-notif', { detail : { message : 'Les points de la division doivent appartenir à la même figure.' } }));
        ...
        let pointsToDivide = [];
        let firstPoints = this.firstPointIds.map(ptId => app.mainCanvasElem.findObjectById(
          ptId,
          'point',
        ));
        ...
        let commonSegment = app.mainCanvasElem.getCommonSegmentOfTwoPoints(
          pt1.id,
          pt2.id,
        );
        let shape = pt1.shape;
  ...
  _executeAction() {
    ...
    if (this.mode == 'twoPoints') {
      this.pointsToDivide.forEach(pts => {
        ...
        this.segment = app.mainCanvasElem.getCommonSegmentOfTwoPoints(
          this.firstPoint.id,
          this.secondPoint.id,
        );
      ...
      this.segment = app.mainCanvasElem.findObjectById(
        this.segmentId,
        'segment',
      );
      ...
      let vector = app.mainCanvasElem.findObjectById(
        this.vectorId,
        'shape',
      );
